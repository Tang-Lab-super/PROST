...
def feature_selection(adata, save_path=None, selected_gene_name=None, by = 'manual', n_top_genes = 3000, auto_select = False, th = 2):
    if by == "prost":
        pi_score = adata.var["PI"]
        threshold = get_threshold(adata)
        if auto_select:
            for i in range(len(threshold)):   
                sorted_score = pi_score[pi_score > threshold[i]].sort_values(ascending = False)
                # selected_genename = pd.DataFrame(sorted_score.index)
                # save_path = "./result/{}".format(number)
                if not os.path.isdir("{}/selected_gene".format(save_path)):
                    os.makedirs("{}/selected_gene".format(save_path))
                sorted_score.to_csv("{}/selected_gene/filtered_gene_name{}.csv".format(save_path,i+1),header = False)
            selected_genename = pd.read_csv("{}/selected_gene/filtered_gene_name{}.csv".format(save_path,th),header = None)
            selected_genename = selected_genename[0]
        else:
        ...
        assert selected_gene_name is not None,"Please set 'selected_gene_name' as type [pandas.Series]!"
        selected_gene_name = pd.Series([i.upper() for i in list(selected_gene_name)])
        selected_genename = selected_gene_name      
    #
    ...
    # raw_gene_name = adata.var['genename'].values.tolist()
