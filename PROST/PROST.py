...
def cal_prost_index(adata, connect_kernel_size=5, del_rate=0.01, platform = "visium"):
    '''
    ...
    adata = get_sub(adata, connect_kernel_size, platform, del_rate)
    adata = cal_PI(adata,platform)
    print("\nPROST Index calculation completed !!")    
    return adata


def run_prost_clust(adata, SEED, platform=None, init="leiden", n_clusters=5, res=0.5,
                    k_neighbors=7, min_distance=50, key_added="PROST", gnnlayers=2, 
    ...
    adj_norm = preprocess_graph(adj, gnnlayers, norm='sym', renorm=False) 
    for a in adj_norm:
        embed = a.dot(embed)
           
    #--------------------------------------------------------------------------
    PNN = PROST_NN(embed.shape[1], embed.shape[1], cuda = cuda)
    
    PNN.train_(embed, adj, init=init, n_clusters=n_clusters, res=res, 
                tol=tol, lr=lr, max_epochs=max_epochs, seed=SEED)  
