...
from . utils import pre_process, make_image, gene_img_flatten, minmax_normalize
...
def minmax_scaler(adata):
    ...
    for i in trange(len(data)):
        single_count = data[i]
        nor_counts[i, :] = minmax_normalize(single_count)    
    adata.uns['nor_counts'] = nor_counts  
    return adata


def gau_filter_for_single_gene(gene_data, locates, grid_size, platform = "visium", image_idx_1d = None):
    """
    ...
    """ 
    if platform=="visium" or platform=="ST":
        I,_ = make_image(gene_data, locates, platform, grid_size=grid_size)  
...
def gau_filter(adata, platform="visium"):
    ...
    #--------------------------------------------------------------------------
    if platform=="visium" or platform=="ST":
        image_idx_1dd = adata.obs['image_idx_1d'].astype(int)
        gau_fea = np.zeros(gene_data.shape)
        for i in trange(len(gene_data)):
            gau_fea[i, :] = gau_filter_for_single_gene(gene_data[i], locates, grid_size, platform, image_idx_1dd)
            
    #--------------------------------------------------------------------------
    else:
        gau_fea = np.zeros((gene_data.shape[0],adata.uns['shape'][0]*adata.uns['shape'][1]))
        for i in trange(len(gene_data)):
            gau_fea[i, :] = gau_filter_for_single_gene(gene_data[i], locates, grid_size, platform)      
...
def get_binary(adata, platform="visium", method = "iterative"):
    ...
    if platform=="visium" or platform=="ST":
        ...
        for i in trange(len(gene_data)):
            fig1 = gene_data[i, :]
            Im, _ = make_image(fig1, locates, platform, grid_size=grid_size)
            if method == "iterative":    
                    m, n = Im.shape       
                    zd = float(np.max(Im))
                    zx = float(np.min(Im))
...
def get_sub(adata, kernel_size = 5, platform="visium",del_rate = 0.01): 
    ...
    if platform=="visium" or platform=="ST":
        ...
        for i in trange(len(gene_data)):
            ...
            temp_i, _ = make_image(temp_data, locates, grid_size=grid_size)      
...
def cal_prost_index(adata, platform="visium"):
    ...
    #--------------------------------------------------------------------------
        ...
        for i in trange(len(data)): 
            temp_raw = data[i, :]
            temp_img,_ = make_image(temp_raw, locates, platform, grid_size=grid_size)
